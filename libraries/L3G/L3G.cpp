...
void L3G::read()
{
  ...
  Wire.send(L3G_OUT_X_L | (1 << 7));
  Wire.endTransmission();
  Wire.requestFrom(address, (byte)6);

  while (Wire.available() < 6);

  uint8_t xlg = Wire.receive();
  uint8_t xhg = Wire.receive();
  uint8_t ylg = Wire.receive();
  uint8_t yhg = Wire.receive();
  uint8_t zlg = Wire.receive();
  uint8_t zhg = Wire.receive();
